        ...
        children: [
          Transform.scale(
            scale: controller!.value.isFullScreen
                ? (controller!.flags.constantScale
                    ? 1
          ...
          if (!controller!.flags.hideThumbnail)
            if (controller!.value.playerState == PlayerState.preLoading &&
                ...
                !controller!.metadata.isFirstVideo)
              Container(
                color: Colors.black,
              )
            else if ((controller!.value.playerState == PlayerState.playing ||
                    controller!.value.playerState == PlayerState.buffering) &&
                controller!.value.position.inMilliseconds <= 2000 &&
                controller!.flags.showMask)
              Container(
                color: Colors.white,
                child: Center(
                  child: Image.asset(
                    'assets/pustack_color_logo.png',
              ...
              Opacity(
                opacity: controller!.value.isPlaying ||
                        (controller!.value.playerState == PlayerState.paused &&
                            controller!.value.position.inSeconds > 1)
                    ? 0
                    : 1,
              ...
              child: IgnorePointer(
                ...
                child: AnimatedOpacity(
                  ...
                  child: controller!.flags.isLive ||
                          controller!.flags.isRecorded ||
                          controller!.flags.isReplay
                      ? LiveBottomBar(
                          progressBarColors: widget.progressColors!,
                          liveUIColor: widget.liveUIColor,
                          isReplay: controller!.flags.isReplay)
                      : Padding(
